...
/*global
VersionData, getCivData, getWonderResources, CivObj, isValid, valOf, prettify, dataset, onIncrement, onPurchase,
onInvade, document, ui, upgradeData, getUpgradeRowText, updateRequirements, updateResourceTotals, updatePopulation,
updateResourceRows, updateBuildingButtons, updateJobButtons, updatePartyButtons, updateUpgrades, updateDevotion,
updateDeity, gameLog, updateAchievements, setAutosave, setCustomQuantities, textSize, setDelimiters, setShadow,
setNotes, setWorksafe, setIcons, matchType, adjustMorale, killUnit, getCustomNumber, updateTargets, calcArithSum,
logSearchFn, updateMorale, renameWonder, updateWonder, prompt, updateTrader, DOMException, read_cookie, mergeObj,
deleteCookie, localStorage, LZString, importByInput, alert, confirm, window, updateAfterReset, computer, rndRound,
doSiege, getWonderProduction, doAutopilot, doTuning, updateAll, indexArrayByAttr, addAchievementRows, addRaidRows,
addWonderSelectText
*/
...
function getReqText(costObj, qty) {
    ...
    var num, text = "";
    Object.keys(costObj).forEach(function (i) {
...
function meetsPrereqs(prereqObj) {
    ...
    Object.keys(prereqObj).forEach(function (i) {
...
function getCostNote(civObj) {
    ...
    var reqText = getReqText(civObj.require),
        effectText = (isValid(civObj.effectText)) ? civObj.effectText : "",
        separator = (reqText && effectText) ? ": " : "";
...
function getResourceRowText(purchaseObj) {
    ...
    objId = purchaseObj.id;
    objName = purchaseObj.getQtyName(0);
    s = (
...
function getPurchaseCellText(purchaseObj, qty, inTable) {
    ...
    tagName = inTable ? "td" : "span";
    className = (abs(qty) === "custom") ? "buy" : purchaseObj.type;  // 'custom' buttons all use the same class.

    s = "<" + tagName + " class='" + className + abs(qty) + "' data-quantity='" + qty + "' >";
    if (allowPurchase()) {
...
function getPurchaseRowText(purchaseObj) {
    ...
    objId = purchaseObj.id;
    s = "<tr id='" + objId + "Row' class='purchaseRow' data-target='" + purchaseObj.id + "'>";

    [-Infinity, "-custom", -100, -10, -1].forEach(function (elem) {
        s += getPurchaseCellText(purchaseObj, elem);
    });

    enemyFlag = (purchaseObj.alignment === "enemy") ? " enemy" : "";
    s += "<td class='itemname" + enemyFlag + "'>" + purchaseObj.getQtyName(0) + ": </td>";

    action = (isValid(population[objId])) ? "display_pop" : "display"; //xxx Hack
...
function addUITable(civObjs, groupElemName) {
    ...
    groupElem = document.getElementById(groupElemName);
...
function getUpgradeRowText(upgradeObj, inTable) {
    "use strict";
    var cellTagName, rowTagName, s;

    if (inTable === undefined) {
        inTable = true;
    }
    cellTagName = inTable ? "td" : "span";
    rowTagName = inTable ? "tr" : "span";
    // Make sure to update this if the number of columns changes.
    if (!upgradeObj) {
        return inTable ? "<" + rowTagName + " class='purchaseRow'><td colspan='2'/>&nbsp;</" + rowTagName + ">" : "";
    }

    s =  "<" + rowTagName + " id='" + upgradeObj.id + "Row' class='purchaseRow'";
...
function getPantheonUpgradeRowText(upgradeObj) {
    ...
    s = "<tr id='" + upgradeObj.id + "Row' class='purchaseRow'>";
...
function setPantheonUpgradeRowText(upgradeObj) {
    ...
    elem = document.getElementById(upgradeObj.id + "Row");
    if (!elem) {
        return null;
    }
...
function getDeityRowText(deityId, deityObj) {
    ...
    return "<tr id='" + deityId + "'>"
        ...
        + "<span id=" + deityId + "Domain' class='deityDomain'>" + "</td><td>" + idToType(deityObj.domain)
        + "</span></td>" + "<td><span id='" + deityId + "Devotion'>" + deityObj.maxDev + "</span></td></tr>";
}

function makeDeitiesTables() {
    "use strict";
    // Display the active deity
    var deityId = "deityA", s;
    ...
    s = "<tr><td><b>Name</b></td><td><b>Domain</b></td><td><b>Max Devotion</b></td></tr>";
...
function increment(objId) {
    "use strict";
    var purchaseObj = civData[objId], numArmy = 0, specialChance,
        specialMaterial, specialQty;
    ...
    specialChance = purchaseObj.specialChance;
    if (specialChance && purchaseObj.specialMaterial && civData[purchaseObj.specialMaterial]) {
        ...
        if (Math.random() < specialChance) {
            specialMaterial = civData[purchaseObj.specialMaterial];
            specialQty =  purchaseObj.increment * (1 + (9 * (civData.guilds.owned)));
...
function onPurchase(control) {
    ...
    var targetId = dataset(control, "target"), qty;
    if (targetId === null) {
        return false;
    }

    qty = dataset(control, "quantity");
    if (qty === null) {
        return false;
    }
...
function getCustomNumber(civObj) {
    ...
    elem = document.getElementById(civObj.customQtyId);
    if (!elem) {
        return undefined;
    }
...
function pickStarveTarget() {
    ...
    var modNum, jobNum,
        modList = ["ill", "owned"],
        jobList = ["unemployed", "blacksmith", "tanner", "miner", "woodcutter",
            "cleric", "cavalry", "soldier", "healer", "labourer", "farmer"];

    for (modNum = 0; modNum < modList.length; modNum += 1) {
        for (jobNum = 0; jobNum < jobList.length; jobNum += 1) {
...
function summonShade() {
    ...
    num = Math.ceil(curCiv.enemySlain.owned / 4 + (Math.random() * curCiv.enemySlain.owned / 4));
...
function randomHealthyWorker() {
    "use strict";
    var num, chance, i;
    num = Math.random() * population.healthy;
    chance = 0;
    for (i = 0; i < killable.length; i += 1) {
...
function wickerman() {
    "use strict";
    var job, rewardObj, qty;
    //Select a random worker
    job = randomHealthyWorker();
    if (!job) {
        return;
    }
    ...
    rewardObj = lootable[Math.floor(Math.random() * lootable.length)];
...
function iconoclasmList() {
    ...
    if (civData.piety.owned >= 1000) {
        ...
        append = "<br />";
        for (i = 1; i < curCiv.deities.length; i += 1) {
...
function smiteMob(mobObj) {
    ...
    num = Math.min(mobObj.owned, Math.floor(civData.piety.owned / 100));
...
function adjustMorale(delta) {
    ...
    if (population.current > 0) { //dividing by zero is bad for hive
        ...
        fraction = population.living / population.current;
        max = 1 + (0.5 * fraction);
        min = 1 - (0.5 * fraction);
        //alters morale
...
function renameWonder() {
    ...
    n = prompt("Please name your Wonder:", curCiv.curWonder.name);
    if (!n) {
        return;
    }
    curCiv.curWonder.name = n;
    wp = ui.find("#wonderNameP");
    if (wp) {
        wp.innerHTML = curCiv.curWonder.name;
    }
    wc = ui.find("#wonderNameC");
    if (wc) {
...
function startTrader() {
    ...
    tradeItems = [ // Item and base amount
    ...
    selected = tradeItems[Math.floor(Math.random() * tradeItems.length)];
...
function trade() {
    ...
    material = civData[curCiv.trader.materialId];

    material.owned -= curCiv.trader.requested;
    civData.gold.owned += 1;
    updateResourceTotals();
...
function load(loadType) {
    ...
    var loadVar = {},
        loadVar2 = {},
        settingsVar = {},
        saveVersion = new VersionData(1, 0, 0, "legacy"),
        string1,
        string2,
        settingsString,
        alertStr,
    ...
    if (saveVersion.toNumber() > versionData.toNumber()) {
        ...
        alertStr = "Cannot load; saved game version " + saveVersion + " is newer than game version " + versionData;
...
function importByInput(elt) {
    ...
    var compressed = elt.value,
        ...
        revived = JSON.parse(decompressed),
    //set variables to load from
        loadVar = revived[0],
        loadVar2;
...
function save(savetype) {
    ...
    var saveVar = {
        ...
        settingsVar = settings, // UI Settings are saved separately.
        savestring,
        compressed;
    ...
    // Handle export
    if (savetype === "export") {
        savestring = "[" + JSON.stringify(saveVar) + "]";
        compressed = LZString.compressToBase64(savestring);
...
function haveDeity(name) {
    "use strict";
    var i;
    for (i = 0; i < curCiv.deities.length; i += 1) {
...
function reset() {
    ...
    //Check player really wanted to do that.
    var msg = "Really reset? You will keep past deities and wonders (and cats)";
    if (!confirm(msg)) {
        return false;
    } // declined
...
function doFarmers() {
    "use strict";
    var nets = computer.calculate_farmers(true);
...
function doWoodcutters() {
    "use strict";
    var nets = computer.calculate_woodcutters(true);
...
function doMiners() {
    "use strict";
    var nets = computer.calculate_miners(true);
...
function doBlacksmiths() {
    "use strict";
    var nets, ore_used, metal_earned;
    nets = computer.calculate_blacksmiths();
    ore_used = nets.ore_used;
    metal_earned = nets.metal_earned;
...
function doTanners() {
    "use strict";
    var nets, skins_used, leather_earned;
    nets = computer.calculate_tanners();
    skins_used = nets.skins_used;
    leather_earned = nets.leather_earned;
...
function doClerics() {
    "use strict";
    var nets, piety_earned;
    nets = computer.calculate_clerics();
    piety_earned = nets.piety_earned;
...
function getNextPatient() {
    "use strict";
    var i;
    for (i = 0; i < PATIENT_LIST.length; i += 1) {
...
function doHealers() {
    "use strict";
    var job, numHealed = 0,
        numHealers = civData.healer.owned + (civData.cat.owned * (civData.companion.owned));
    ...
    while (civData.healer.cureCount >= 1 && civData.herbs.owned >= 1) {
        ...
        healByJob(job);
        civData.healer.cureCount -= 1;
        civData.herbs.owned -= 1;
        numHealed += 1;
    }
...
function doPlague() {
    ...
    var jobInfected = getRandomPatient(),
        unitInfected = civData[jobInfected],
        deathRoll = (100 * Math.random()) + 1;
...
function doGraveyards() {
    ...
    if (civData.corpses.owned > 0 && curCiv.grave.owned > 0) {
        ...
        for (i = 0; i < civData.cleric.owned; i += 1) {
            if (civData.corpses.owned > 0 && curCiv.grave.owned > 0) {
...
function doFight(attacker, defender) {
    "use strict";
    var fortMod, palisadeMod, attackerCas, defenderCas, playerCredit;
    if ((attacker.owned <= 0) || (defender.owned <= 0)) {
        return;
    }

    // Defenses vary depending on whether the player is attacking or defending.
    fortMod = (defender.alignment === "player" ? (civData.fortification.owned * civData.fortification.efficiency)
                                                   : (civData.efort.owned * civData.efort.efficiency));
    palisadeMod = ((defender.alignment === "player") && (civData.palisade.owned)) * civData.palisade.efficiency;
    ...
    attackerCas = Math.min(attacker.owned, rndRound(getCasualtyMod(defender, attacker) * defender.owned * defender.efficiency));
    defenderCas = Math.min(defender.owned, rndRound(getCasualtyMod(attacker, defender) * attacker.owned * (attacker.efficiency - palisadeMod) * Math.max(1 - fortMod, 0)));

    attacker.owned -= attackerCas;
    defender.owned -= defenderCas;

    // Give player credit for kills.
    playerCredit = ((attacker.alignment === "player") ? defenderCas :
...
function doSlaughter(attacker) {
    "use strict";
    var killVerb, target, targetUnit, leaving;
    killVerb = (attacker.species === "animal") ? "eaten" : "killed";
    target = randomHealthyWorker(); //Choose random worker
    targetUnit = civData[target];
    if (target) {
        ...
        leaving = Math.ceil(attacker.owned * Math.random() * attacker.killFatigue);
...
function doLoot(attacker) {
    "use strict";
    var target, stolenQty, leaving;
    // Select random resource, steal random amount of it.
    target = lootable[Math.floor(Math.random() * lootable.length)];
    ...
    if (target.owned <= 0) {
        //some will leave
        leaving = Math.ceil(attacker.owned * Math.random() * attacker.lootFatigue);
        attacker.owned -= leaving;
    }
...
function doSack(attacker) {
    "use strict";
    var target, destroyVerb, leaving;
    //Destroy buildings
    target = sackable[Math.floor(Math.random() * sackable.length)];
...
function doRaid(place, attackerID, defenderID) {
    ...
    attackers = getCombatants(place, attackerID);
    defenders = getCombatants(place, defenderID);
...
function getWonderLowItem() {
    "use strict";
    var lowItem = null, i = 0;
    for (i = 0; i < wonderResources.length; i += 1) {
...
function doMobs() {
    ...
    getCombatants("home", "enemy").forEach(function (attacker) {
        ...
        defenders = getCombatants(attacker.place, "player");
        if (!defenders.length) {
            attacker.onWin();
            return;
        } // Undefended
...
function tickTraders() {
    "use strict";
    var check, delayMult = 60 * (3 - ((civData.currency.owned) + (civData.commerce.owned)));
    //traders occasionally show up
    if (population.current > 0) {
        curCiv.trader.counter += 1;
    }
...
function paneSelect(control) {
    "use strict";
    var i, oldTarget, newTarget, selectors, curSelects, targetElem;

    // Identify the target pane to be activated, and the currently active
    // selector tab(s).
    newTarget = dataset(control, "target");
    selectors = ui.find("#selectors");
    if (!selectors) {
        console.log("No selectors found");
        return null;
    }
    curSelects = selectors.getElementsByClassName("selected");

    // Deselect the old panels.
    for (i = 0; i < curSelects.length; i += 1) {
    ...
    targetElem = document.getElementById(newTarget);
    if (targetElem) {
...
function setCustomQuantities(value) {
    "use strict";
    var i, elems, curPop = population.current;
...
function setShadow(value) {
    ...
    shadowStyle = "3px 0 0 #fff, -3px 0 0 #fff, 0 3px 0 #fff, 0 -3px 0 #fff"
...
function gameLog(message) {
    ...
    var d = new Date(),
        curTime = d.getHours() + ":" + ((d.getMinutes() < 10) ? "0" : "") + d.getMinutes(),
        i = 7,
        s;
    ...
    if (ui.find("#logL").innerHTML !== message) {
        ...
        i = 6; // Number of lines of log to keep.
        while (i >= 1) {
    ...
    s =  "<td id='logT'>" + curTime + "</td><td id='logL'>" + message + "</td><td id='logR'>";
    logRepeat += 1;
    if (logRepeat > 1) {
        s += "(x" + logRepeat + ")";
    } // Optional (xNumber)
...
setup.events = function () {
    ...
    openSettingsElt.addEventListener("click", function () {
        settingsShown = ui.toggle("#settings");
