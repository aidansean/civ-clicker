...
/*global
updateTrader, updateUpgrades, updateResourceRows, updateBuildingButtons, updateJobButtons, updatePartyButtons,
updatePopulation, updateTargets, updateDevotion, updateWonder, updateReset, civData, ui, updateResourceTotals,
updateDeity, makeDeitiesTables, curCiv, updateRequirements, prettify, isTraderHere, document, getReqText, meetsPrereqs,
canPurchase, dataset, abs, sgn, basicResources, homeBuildings, homeUnits, armyUnits, settings, getLandTotals, isValid,
calculatePopulation, population, getCivType, logSearchFn, calcWorkerCost, getCurDeityDomain, canRaise, calcZombieCost,
calcWorkerCost, updateJobButtons, updatePartyButtons, updateMorale, updateAchievements, updatePopulationBar,
updateLandBar, unitData, upgradeData, idToType, buildingData, canAfford, powerData, achData, civSizes, onBulkEvent,
getCombatants, wonderResources, isWonderLimited, getWonderLowItem
*/
...
function updateWonderList() {
    ...
    //update wonder list
    var i,
        wonderhtml = "<tr><td><strong>Name</strong></td><td><strong>Type</strong></td></tr>";
    for (i = (curCiv.wonders.length - 1); i >= 0; i -= 1) {
        try {
...
function updateTrader() {
    ...
    if (isHere) {
        ...
        ui.find("#traderTimer").innerHTML = curCiv.trader.timer + " second" + suffix;
    }
...
function updatePurchaseRow(purchaseObj) {
    ...
    havePrereqs = (purchaseObj.owned > 0) || meetsPrereqs(purchaseObj.prereqs);

    // Special check: Hide one-shot upgrades after purchase; they're
    // redisplayed elsewhere.
    hideBoughtUpgrade = ((purchaseObj.type === "upgrade") && (purchaseObj.owned === purchaseObj.limit) && !purchaseObj.salable);

    maxQty = canPurchase(purchaseObj);
    minQty = canPurchase(purchaseObj, -Infinity);

    buyElems = elem.querySelectorAll("[data-action='purchase']");
...
function updateResourceTotals() {
    ...
    for (i = 0; i < displayElems.length; i += 1) {
        elem = displayElems[i];
    ...
    Object.keys(displayElems).forEach(function (index) {
        i = parseInt(index, 10);
        elem = displayElems[i];
    ...
    if ((civData.temple.owned > 0) || (curCiv.deities.length > 1)) {
        ui.show("#deitySelect", true);
    }
    if (civData.barracks.owned > 0) {
        ui.show("#conquestSelect", true);
    }
    if (civData.gold.owned > 0) {
        ui.show("#tradeSelect", true);
    }
...
function updatePopulation(calc) {
    ...
    civData.house.update(); // Waiting: Effect might change dynamically.  Need a more general way to do this.
    civData.barn.update();
    ...
    maxSpawn = Math.max(0, Math.min((population.limit - population.living), logSearchFn(calcWorkerCost, civData.food.owned)));
    ...
    canRaise = (getCurDeityDomain() === "underworld" && civData.devotion.owned >= 20);
    maxRaise = canRaise ? logSearchFn(calcZombieCost, civData.piety.owned) : 0;
...
function updateLandBar() {
    ...
    var barElt = ui.find("#landBar"),
        landTotals = getLandTotals(),
        p = (Math.floor(1000 * (landTotals.buildings / landTotals.lands)) / 10);
...
function updateUpgrades() {
    ...
    var domain = getCurDeityDomain(),
        hasDomain = (getCurDeityDomain() === "") ? false : true,
        canSelectDomain = ((civData.worship.owned) && !hasDomain);
...
function updateTargets() {
    "use strict";
    var i,
        raidButtons = document.getElementsByClassName("raid"),
        haveArmy = false,
        curElem;
    ...
    if (ui.show("#raidGroup", !curCiv.raid.raiding)) {
        ...
        for (i = 0; i < raidButtons.length; i += 1) {
...
function updateWonder() {
    ...
    var haveTech = (civData.architecture.owned && civData.civilservice.owned),
        isLimited = isWonderLimited(),
        lowItem = getWonderLowItem();
